...
test('wide Reader modes replace governed regions without history or query work', async ({ page }) => {
  const { read, reader } = await openReader(page);
  const mode = reader.getByRole('group', { name: 'Reader width' });
  ...
  const peek = mode.getByRole('button', { name: 'peek', exact: true });
  await peek.click();
  await expect(peek).toBeFocused();
  await expect(peek).toHaveAttribute('aria-pressed', 'true');
  await expect(reader).toHaveAttribute('data-mode', 'peek');
  await expect(reader).toHaveAttribute('data-slot', 'evidence');
  await expect(page.getByRole('complementary', { name: 'Evidence' })).toHaveCount(0);
  await expect(page.getByRole('region', { name: 'Trends', exact: true })).toHaveCount(1);
  ...
  await mode.getByRole('button', { name: 'peek', exact: true }).click();
  await page.goBack();
  await expect(reader).toHaveCount(0);
  await expect(read).toBeFocused();
  await page.goForward();
  const restored = page.getByRole('dialog', { name: /Reader:/ });
  await expect(restored).toHaveAttribute('data-requested', 'peek');
});

test('Reader retains one DOM and requested mode across every width class', async ({ page }) => {
  ...
  await reader.evaluate((element) => {
    element.dataset.ttProbe = 'reader-stays-mounted';
  });
  const mark = (await trace(page)).events.at(-1)?.seq ?? -1;

  await page.setViewportSize({ width: 768, height: 1024 });
  await expect(reader).toHaveAttribute('data-mode', 'study');
  await expect(reader).toHaveAttribute('data-requested', 'peek');
  ...
  await expect(page.getByRole('complementary', { name: 'Queries' })).toHaveCount(0);
  await expect(page.getByRole('region', { name: 'Trends', exact: true })).toHaveCount(0);
  await expect(page.getByRole('complementary', { name: 'Evidence' })).toHaveCount(0);
