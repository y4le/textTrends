...
export function ReaderAtlas({
  ...
  const columnDetails = useMemo(() => columns.map((column) => {
    ...
    return {
      title,
      description: atlasColumnDescription(column, title, labels),
      coarse: column.tracks.flatMap((track) => {
        const disclosure = atlasCoarseDisclosure(
          track,
          labels.get(track.seriesId) ?? track.seriesId,
        );
