 ...
 *
 * The store is exported synchronously, while its IndexedDB-backed project is
 * restored asynchronously. The session is attached once: construct the session
...
async function bootstrap(): Promise<void> {
  ...
  try {
    const [
      { localLibrary },
      { libraryProject, reconcileLibraryWorkspace },
      { ProjectSession },
    ] = await Promise.all([
