...
export function provenanceFor(input: ProvenanceInput, place: Place): ProvenanceV1 {
  ...
  if (place === 'corpus' && input.inventory) {
    ...
    methods = [{
      method: 'research log',
      parameters: [
        parameter('evidence identity', 'captured snapshot, document, position, and query tracks'),
        parameter('source policy', 'source text remains on this device unless explicitly exported'),
      ],
      limitations: [
        'A restored anchor can require review after its source text changes.',
      ],
    }];
